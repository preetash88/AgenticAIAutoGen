...
os.environ[
    "OPENAI_API_KEY"] = ""
os.environ["JIRA_URL"] = "https://abcxyz.atlassian.net/"
os.environ["JIRA_USERNAME"] = "xyz@gmail.com"
os.environ["JIRA_PROJECTS_FILTER"] = "ABC"
os.environ[
    "JIRA_API_TOKEN"] = ""
...
async def function_one():
    ...
    jira_server_params = StdioServerParams(command="docker", args=["run",
                                                                   "-i",
                                                                   "--rm",
                                                                   ...
                                                                   "-e",
                                                                   f"JIRA_API_TOKEN={os.environ['JIRA_API_TOKEN']}",
                                                                   "-e",
    ...
    playwright_tools = await mcp_server_tools(playwright_server_params)

    # Create an agent that can use all the tools
    model_client = OpenAIChatCompletionClient(
        model="gpt-4o"
    )
    jira_assistant = AssistantAgent(name="JIRAAgent", model_client=model_client, tools=jira_tools,
    ...
    playwright_assistant = AssistantAgent(name="PlaywrightAgent", model_client=model_client, tools=playwright_tools,
    ...
    prompts = RoundRobinGroupChat(participants=[jira_assistant, playwright_assistant],
    ...
    await Console(prompts.run_stream(
        task="JIRAAgent: \n"
             ...
             "PlaywrightAgent:\n"
             "Once ready, automate this flow using Playwright MCP and execute it."))  # run_stream gives the output in the console line by line. But run will just give you a cumulative result at the end.
    await jira_assistant.close()
    await playwright_assistant.close()
