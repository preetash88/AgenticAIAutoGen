...
async def model():
    ...
    database_agent = await factory.create_database_agent(
        system_message="You are a Database specialist responsible for retrieving user registration data."
    ...
    api_agent = await factory.create_api_agent(
        system_message="You are an API testing specialist with access to both REST API tools and filesystem."
    ...
    excel_agent = await factory.create_api_agent(
        system_message="You are an Excel data management specialist. ONLY proceed when APIAgent has completed testing."
    ...
    multi_agents = RoundRobinGroupChat(participants=[database_agent, api_agent, excel_agent],
                                       termination_condition=TextMentionTermination("REGISTRATION PROCESS COMPLETE"))

    await Console(multi_agents.run_stream(task="Execute Sequential User Registration Process:\n\n"
